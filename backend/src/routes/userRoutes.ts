...
routes.route(routeNames.supplierSignup).post(userController.supplierSignup)
...
routes.route(routeNames.createSupplierDoc).post([multer({ storage: multer.memoryStorage() }).single('file')], userController.createSupplierDoc)
routes.route(routeNames.deleteTempSupplierDoc).post(userController.deleteTempSupplierDoc)
