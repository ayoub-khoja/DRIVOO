...
const sameWebOrigin = (): boolean =>
  helper.trimEnd(env.ADMIN_HOST, '/') === helper.trimEnd(env.FRONTEND_HOST, '/')

const appTypeHeader = (req: Request): string =>
  String(req.headers[env.X_APP_TYPE] || '').toLowerCase()

/**
 * Check whether the request is from the admin or not.
 */
export const isAdmin = (req: Request): boolean => {
  const app = appTypeHeader(req)
  if (app === bookcarsTypes.AppType.Admin) {
    return true
  }
  if (app === bookcarsTypes.AppType.Frontend || app === bookcarsTypes.AppType.Agency) {
    return false
  }
  // Same origin: require X-BC-App to avoid cookie collisions
  if (sameWebOrigin()) {
    return false
  }
  return !!req.headers.origin && helper.trimEnd(req.headers.origin, '/') === helper.trimEnd(env.ADMIN_HOST, '/')
}
...
export const isFrontend = (req: Request): boolean => {
  const app = appTypeHeader(req)
  if (app === bookcarsTypes.AppType.Frontend) {
    return true
  }
  if (app === bookcarsTypes.AppType.Admin || app === bookcarsTypes.AppType.Agency) {
    return false
  }
  if (sameWebOrigin()) {
    return !!req.headers.origin && helper.trimEnd(req.headers.origin, '/') === helper.trimEnd(env.FRONTEND_HOST, '/')
  }
  return !!req.headers.origin && helper.trimEnd(req.headers.origin, '/') === helper.trimEnd(env.FRONTEND_HOST, '/')
}

/**
 * Check whether the request is from the agency portal.
 */
export const isAgency = (req: Request): boolean => appTypeHeader(req) === bookcarsTypes.AppType.Agency
...
export const getAuthCookieName = (req: Request): string => {
  ...
  if (isAgency(req)) {
    return env.AGENCY_AUTH_COOKIE_NAME
  }
...
/**
 * Resolve cookie name from AppType (sign-in routes).
 */
export const getAuthCookieNameByAppType = (type: bookcarsTypes.AppType): string => {
  if (type === bookcarsTypes.AppType.Admin) {
    return env.ADMIN_AUTH_COOKIE_NAME
  }
  if (type === bookcarsTypes.AppType.Agency) {
    return env.AGENCY_AUTH_COOKIE_NAME
  }
  return env.FRONTEND_AUTH_COOKIE_NAME
}
