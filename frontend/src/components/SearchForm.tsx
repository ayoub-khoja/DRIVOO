...
const SearchForm = ({
  ...
  // Use fallback settings so the home search UI still renders without API
  const minPickupHours = settings?.minPickupHours ?? 1
  ...
  useEffect(() => {
    ...
    _minTime.setHours(minPickupDropoffHour, 0, 0, 0)
    setMinTime(_minTime)

    const _maxTime = new Date()
    _maxTime.setHours(maxPickupDropoffHour, 0, 0, 0)
    setMaxTime(_maxTime)

    let _minDate = new Date()
    _minDate = addHours(_minDate, minPickupHours)

    setFromMinDate(_minDate)
    setMinDate(_minDate)
  }, [minPickupHours, minPickupDropoffHour, maxPickupDropoffHour])
  ...
  useEffect(() => {
    ...
    if (minPickupHours < 72) {
      _from.setDate(_from.getDate() + 3)
    } else {
      _from.setDate(_from.getDate() + Math.ceil(minPickupHours / 24) + 1)
    }
    _from.setHours(10)
    _from.setMinutes(0)
    _from.setSeconds(0)
    ...
    if (minRentalHours < 72) {
      _to.setDate(_to.getDate() + 3)
    } else {
      _to.setDate(_to.getDate() + Math.ceil(minRentalHours / 24) + 1)
    }

    let __minDate = new Date()
    __minDate = addHours(__minDate, minRentalHours)

    setMinDate(__minDate)
    setValue('from', _from)
    setValue('to', _to)
  }, [minPickupHours, minRentalHours]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const validateHour = (hour: number) => hour >= minPickupDropoffHour && hour <= maxPickupDropoffHour

  const validateTimes = () => {
    if (!fromMinDate || !minDate || !minTime || !maxTime) {
      return false
    }

    let valid = true
    const minPickupDuration = minPickupHours * 60 * 60 * 1000
    const minRentalDuration = minRentalHours * 60 * 60 * 1000
    ...
    if (from && to) {
      ...
      if (from.getTime() > to.getTime()) {
        ...
        if (!fromHourValid) {
          _from.setHours(minPickupDropoffHour)
          setValue('from', _from)
        }
        const toHourValid = validateHour(_to.getHours())
        if (!toHourValid) {
          _to.setHours(minPickupDropoffHour)
        }
        setValue('to', _to)
  ...
  // Wait until date bounds are ready
  if (!minDate || !fromMinDate || !minTime || !maxTime) {
    return null
  }
