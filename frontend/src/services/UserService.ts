 ...
 * @returns {Promise<number>}
 */
export const supplierSignup = (data: bookcarsTypes.SignUpPayload): Promise<number> =>
  axiosInstance
    .post(
      '/api/supplier-sign-up',
      data
    )
    .then((res) => res.status)

/**
 * Upload a supplier application document (RNE) to temp storage.
 */
export const createSupplierDoc = (file: Blob): Promise<string> => {
  const formData = new FormData()
  formData.append('file', file)

  return axiosInstance
    .post(
      '/api/create-supplier-doc',
      formData,
      {
 ...
 */
export const deleteTempSupplierDoc = (fileName: string): Promise<number> =>
  axiosInstance
    .post(`/api/delete-temp-supplier-doc/${encodeURIComponent(fileName)}`)
    .then((res) => res.status)
