...
const Offer = () => {
  ...
  const ready = visible && !!car && !!pickupLocation && !!dropOffLocation && !!from && !!to
  const locale = getDateFnsLocale(language)
  const pickupTime = ready && from ? format(from, 'HH:mm', { locale }) : ''
  const carImageUrl = ready && car?.image ? bookcarsHelper.joinURL(env.CDN_CARS, car.image) : ''
  const bookingScore = ready && car ? formatBookingRating(car.rating) : ''
  const ratingLabel = ready && car ? getRatingLabel(car.rating, language) : ''
  const similarLabel = ready && car ? getSimilarCategoryLabel(car.range, language) : ''
  const bagCount = ready && car ? Math.max(1, Math.floor(car.seats / 2)) : 1
  const hasFreeCancellation = ready && car ? car.cancellation === 0 : false

  const highlights = ready && car ? [
  ...
  return (
    <Layout onLoad={onLoad} strict={false}>
      {!ready || !car || !pickupLocation || !dropOffLocation || !from || !to ? (
        <Progress />
      ) : (
